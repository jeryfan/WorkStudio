 ...
 *
 * 之前它是个**死按钮**:类名摆好了但没挂任何 handler,点下去什么都不发生
 * (D6 记了它"不是 Radix trigger、缺 aria-haspopup/aria-expanded")。
 * 实测确认过不是被什么东西盖住 —— `elementFromPoint` 命中的就是这个 button,
 * `-webkit-app-region` 是 none、`pointer-events` 是 auto,header 那层是
 * `pointer-events-none` 不吃事件。纯粹是没接。
 *
 * 现在按 Codex 实测补齐(点开它的菜单抓的 DOM):
 *
 * ```
 * button[aria-haspopup=menu][aria-expanded][data-state][aria-label="Switch mode, current mode: X"]
 * div[role=menu][data-radix-menu-content][data-side=bottom][data-align=start][data-state=open]
 *   .z-50.w-[240px].m-px.flex.flex-col.overflow-y-auto.p-1.5.px-1.py-1.no-drag.select-none
...
const MENU_ITEM =
  'group flex cursor-interaction flex-col rounded-lg px-[var(--padding-row-x)] py-2.5 py-[var(--padding-row-y)] ' +
  'text-base text-sm text-token-foreground no-drag outline-hidden ' +
  'hover:bg-token-list-hover-background focus:bg-token-list-hover-background'
...
export function SidebarModeSwitcher(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent): void => {
      ...
      if (menuEl?.contains(t) === true) return
      if (triggerEl?.contains(t) === true) return
      setOpen(false)
    }
      ...
      {open &&
        createPortal(
          <div
            ...
            dir="ltr"
            tabIndex={-1}
            className="z-50 m-px flex w-[240px] flex-col overflow-y-auto rounded-xl bg-token-dropdown-background/90 p-1.5 px-1 py-1 text-token-foreground no-drag select-none ring-[0.5px] ring-token-border shadow-xl-spread backdrop-blur-sm"
            style={{ ...floating.floatingStyles, outline: 'none' }}
          >
            {MODES.map((mode) => (
              <div
                key={mode.id}
