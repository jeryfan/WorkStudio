              ...
              {rootPaths.length === 0 ? (
                ...
                <div className="flex flex-col gap-1">
                  {rootPaths.map((path) => (
                    <div
                      key={path}
                      className="flex items-center gap-2 rounded-lg px-2 py-1 hover:bg-row-hover"
                    >
                      <SourcesIcon className="size-4 shrink-0 text-desc" />
                      {/* 长路径从头部截断：尾部的目录名才是识别依据 */}
                      <span
                        className="min-w-0 flex-1 truncate text-left text-[13px] text-ink [direction:rtl]"
                        title={path}
                      >
                        {path}
                      </span>
                      <button
                        ...
                        className="shrink-0 rounded p-0.5 text-desc hover:text-ink [&_svg]:size-3.5"
                      >
                        <CloseIcon />
                      </button>
                    </div>
                  ))}
                  <span className="px-2 pt-1 text-[12px] text-desc">Click to add more</span>
                </div>
              )}
            </div>
          ...
          <div className="mt-auto flex w-full items-center justify-between gap-3 pt-5">
            ...
            <div className="flex w-full items-center justify-end gap-3">
              ...
              <button
                ...
                className="flex select-none items-center gap-1 whitespace-nowrap rounded-[12.5px] border border-transparent bg-ink px-4 py-1.5 text-sm leading-[18px] text-dropdown hover:bg-ink/80 disabled:cursor-not-allowed disabled:opacity-40"
              >
                Create project
              </button>
            </div>
